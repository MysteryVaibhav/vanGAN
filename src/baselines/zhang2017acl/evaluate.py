...
def get_translation_dict(source_word_list, target_word_list, knn_indices):
    ...
    for i, word in enumerate(source_word_list):
        # print("%d: %s" % (i, word))
        translation_dict[word] = [target_word_list[j] for j in knn_indices[i]]
    return translation_dict


def get_knn_indices(k, embs_trg, embs_trg_mapped):
    index = faiss.IndexFlatIP(g_input_size)
    index.add(embs_trg)
    distances, knn_indices = index.search(embs_trg_mapped, k)
...
def get_mapped_embeddings(g, source_word_list, gpu=False):
    """"Map source words in the testing pairs into the target language."""
    source_vec_dict, target_vec_dict = get_embeddings_dicts()
    target_word_list = list(target_vec_dict.keys())
    # Embeddings mapped from source words
...
def get_precision_k(k, g, true_dict, method='csls', gpu=False):
    source_word_list = list(true_dict.keys())

    _, embs_trg = get_embeddings(lang_src=lang_src, lang_trg=lang_trg)
    embs_trg = embs_trg.astype(np.float32)  # for faiss
    # Normalize
    embs_trg /= np.linalg.norm(embs_trg, axis=1).reshape((-1, 1))

    embs_trg_mapped, target_word_list = get_mapped_embeddings(g, source_word_list, gpu=gpu)
    embs_trg_mapped = embs_trg_mapped.astype(np.float32)  # for faiss
    # Normalize
    embs_trg_mapped /= np.linalg.norm(embs_trg_mapped, axis=1).reshape((-1, 1))

    if method == 'nn':
        _, knn_indices = get_knn_indices(k, embs_trg, embs_trg_mapped)
    elif method == 'csls':
        knn_indices = CSLS_fast(k, embs_trg, embs_trg_mapped)
    else:
        raise 'Method not implemented: %s' % method

    predicted_dict = get_translation_dict(source_word_list, target_word_list, knn_indices)
