...
import numpy as np
from util import get_embeddings_dicts, get_validation_set, \
...
def get_translation_dict(source_word_list, target_word_list, knn_indices):
    ...
    for (i, word) in enumerate(source_word_list):
        nn_idx = list(knn_indices[i])
        translation_dict[word] = [target_word_list[j] for j in nn_idx]
...
def get_precision_k(k, true_dict, xq, target_word_list):
...
def sanity_check(k, true_dict):
    ...
    xq = np.zeros((len(source_word_list), g_input_size))
    _, target_dict = get_embeddings_dicts()
    target_word_list = [true_dict[source_word][0] for source_word in
                        source_word_list]

    print(target_word_list)

    for (i, target_word) in enumerate(target_word_list):
        xq[i] = target_dict[target_word]

    xq = np.float32(xq)
    row_sum = np.linalg.norm(xq, axis=1)
    xq = xq / row_sum[:, np.newaxis]
    _, knn_indices = get_knn_indices(k, xb, xq)
    predicted_dict = get_translation_dict(source_word_list,
...
def normalize_mat(m):
    m = np.float32(m)
    row_sum = np.linalg.norm(m, axis=1)
    if row_sum.any() == 0:
        print("Oh no!!")
    return m/row_sum[:, np.newaxis]
...
if __name__ == '__main__':
    ...
    print(get_precision_k(k, true_dict, xq, target_word_list))
